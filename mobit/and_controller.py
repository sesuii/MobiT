...
configs = load_config()
appPackage = configs['APP_PACKAGE']
appActivity = configs['APP_ACTIVITY']
...
class AndroidController:
    ...
    def get_screenshot(self, save_path):
        cap_command = f"adb -s {self.device} shell screencap -p " \
                      f"{os.path.join(self.screenshot_dir, 'tmp.png').replace(self.backslash, '/')}"
    ...
    def get_xml(self, save_path):
        dump_command = f"adb -s {self.device} shell uiautomator dump " \
                       f"{os.path.join(self.xml_dir, 'tmp.xml').replace(self.backslash, '/')}"
        pull_command = f"adb -s {self.device} pull " \
                       f"{os.path.join(self.xml_dir, 'tmp.xml').replace(self.backslash, '/')} " \
                       f"{save_path}"
    ...
    def get_info(self):
        ...
        ret = execute_adb(adb_command)
        activities = ret.split('\n')
        for key in activities:
            key = key.strip()
            if appPackage in key:
